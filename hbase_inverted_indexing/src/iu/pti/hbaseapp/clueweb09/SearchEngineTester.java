...
public class SearchEngineTester {
	...
	public static void searchKeyword(String keyword) throws Exception {
		...
		// HTable dataTable = new HTable(hbaseConfig,
		// Constants.CW09_DATA_TABLE_BYTES);
		Table dataTable = hbaseConn.getTable(TableName.valueOf(Constants.CW09_DATA_TABLE_BYTES));
		// HTable indexTable = new HTable(hbaseConfig,
		// Constants.CW09_INDEX_TABLE_BYTES);
		Table indexTable = hbaseConn.getTable(TableName.valueOf(Constants.CW09_INDEX_TABLE_BYTES));
		// HTable prTable = new HTable(hbaseConfig,
		// Constants.CW09_PAGERANK_TABLE_BYTES);
		Table prTable = hbaseConn.getTable(TableName.valueOf(Constants.CW09_PAGERANK_TABLE_BYTES));
		...
		// loop through the document IDs in the row. Recall the schema of the
		// clueWeb09IndexTable:
		// row key: term (keyword), column family: "frequencies", qualifier:
		// document ID, cell value: term frequency in the corresponding document
		int pageCount = 0;
		for (Cell kv : indexRow.listCells()) {
			String pageDocId = null;
			int freq = 0;
			String pageUri = null;
			float pageRank = 0;
			// Write your codes for the main part of implementation here
			// Step 1: get the document ID of one page, as well as the keyword's
			// frequency in that page
			pageDocId = Bytes.toString(CellUtil.cloneQualifier(kv));
			//System.out.println("PageDocId : " + pageDocId);
			freq = Bytes.toInt(CellUtil.cloneValue(kv));
			// Step 2: get the URI of the page from clueWeb09DataTable
			dataTableRow = dataTable.get(new Get(Bytes.toBytes(pageDocId)));
			for (Cell dataTableCell : dataTableRow.listCells()) {
				if (Bytes.toString(CellUtil.cloneQualifier(dataTableCell)).equalsIgnoreCase("URI")) {
					pageUri = Bytes.toString(CellUtil.cloneValue(dataTableCell));
					break;
				}
			}
			// Step 3: get the page rank value of this page from
			// clueWeb09PageRankTable
			prTableRow = prTable.get(new Get(Bytes.toBytes(pageDocId)));
			for (Cell prTableCell : prTableRow.listCells()) {
				pageRank = Bytes.toFloat(CellUtil.cloneQualifier(prTableCell));
				break;
			}
			// END of your code
		...
		System.out.println(
				"Evaluated " + pageCount + " pages in total. Here are the top 20 pages according to page ranks:");
		...
		while (stack.size() > 0) {
			...
			System.out.println("Document ID: " + page.docId + ", URI: " + page.URI + ", page rank: " + page.pageRank
					+ ", word frequency: " + page.termFreq);
		}
	}
