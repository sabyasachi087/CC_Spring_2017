...
public class MBKmeansMapper extends CollectiveMapper<String, String, Object, Object> {
	...
	protected void setup(Context context) throws IOException, InterruptedException {
		...
		this.miniBatchSize = configuration.getInt(KMeansConstants.MIN_BATCH_SIZE, (vectorSize / 2));
	...
	private void computation(Table<DoubleArray> cenTable, Table<DoubleArray> previousCenTable,
	...
	private void runKmeans(List<String> fileNames, Configuration conf, Context context) throws IOException {
		// -----------------------------------------------------
		// Load centroids
		// for every partition in the centoid table, we will use the last
		// element to store the number of points
		...
		for (int iter = 0; iter < iteration; iter++) {
			...
			// compute new partial centroid table using previousCenTable and
			// data points
	...
	private void loadCentroids(Table<DoubleArray> cenTable, int vectorSize, String cFileName,
			Configuration configuration) throws IOException {
		String[] vector = null;
		int partitionId = 0;
		for (String text : this.getRandomBatch(cFileName, configuration)) {
			vector = text.split("\\s+");
			if (vector.length != vectorSize) {
				System.out.println("Errors while loading centroids .");
				System.exit(-1);
			} else {
				batched_data_set.add(vector);
			}
		}
		for (int count = 0; count < this.miniBatchSize; count++) {
			vector = batched_data_set.get(count);
	...
	private Set<String> getRandomBatch(String cFileName, Configuration configuration) {
		...
		try {
			...
			while (lines.size() <= this.miniBatchSize) {
				while ((line = br.readLine()) != null) {
					if (lines.size() == this.miniBatchSize) {
						break;
					} else if (this.chance()) {
						lines.add(line);
					}
	...
	// load data form HDFS
	private ArrayList<DoubleArray> loadData(List<String> fileNames, int vectorSize, Configuration conf)
			throws IOException {
